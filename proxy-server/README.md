...
`start.ps1` is a one-command launcher: it picks a backend, starts the sidecar, waits
for it to become healthy, then starts the proxy **and the admin web UI** (`web/`,
Next.js) on `http://127.0.0.1:3939` — loopback only. The launcher passes the admin
API address and `admin.auth_token` from the chosen config to the UI automatically.
Pass `-NoWeb` to skip the UI. Ctrl+C stops everything it started.
