...
現状は対象モデルが safetensors のみのため `scripts/convert-model-gguf.ps1` でローカル変換し、
`start.ps1 -Model <local.gguf>` で読み込む運用。上流（akiFQC 等）が `*-GGUF` リポジトリを公開したら、
変換を省いて `start.ps1 -Model <repo>-GGUF:<quant>`（`-hf` 自動DL）に切替えられる（既存分岐で対応済み）。

### Unblock condition

対象ファミリの GGUF リポジトリが HuggingFace に公開される。
