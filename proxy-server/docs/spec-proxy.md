...
| 項目 | 内容 |
|---|---|
| 分類タイムアウト(§8.6) | `1500ms → 8000ms`。CPU推論はアイドル後の初回で1.5sを超え、過小だと**正常リクエストをfail-closedで誤ブロック**。warmは200-300ms。GPU/NPUでは下げる |
...
初期設計では、Goプロキシ本体と推論ランタイムを分離する。

```text
Go Local DLP Proxy
  ↓ localhost HTTP / gRPC
Inference Adapter
  ↓ ONNX Runtime / Vitis AI EP等
AMD Ryzen AI NPU
```

理由:

* NPU向けランタイムは依存関係が重く、Go本体に直接組み込むと保守性が下がる。
* CPU/GPU/NPUバックエンドの差し替えを容易にする。
...
```text
classify_timeout_ms: 8000       # 改訂: CPU推論向けに1500→8000。warmは200-300ms。GPU/NPUでは下げる
...
MVPは **llama.cpp サーバ（`llama-server`）** をサイドカーとして localhost で起動し、OpenAI互換 `POST /v1/chat/completions` を叩く。健全性は `GET /health`。NPU(ONNX Runtime + Vitis AI EP)は将来、同じ `Classifier` インターフェースの背後で差し替える。
