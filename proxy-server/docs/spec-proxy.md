...
| 推論I/F(§8.4/§8.5) | gRPC/ONNX抽象 | **OpenAI互換サイドカー**: llama.cpp（`/v1/...`, Vulkan/CPU）／**AMD NPU は自作 Ryzen AI ONNX シム**（`npu/npu_server.py`、同じ `/v1/...`・別ポート8792）。同一HTTP契約でendpoint/モデル/プロファイルのみ切替 | CPU/iGPUで確実に回り、NPUも同じAdapterで載る |
...
| 実行環境/バックエンド | 最終環境は **AMD Ryzen AI シリーズ APU**（RDNA3.5 iGPU + XDNA2 NPU。例: 開発機 Ryzen AI MAX+ 395 / デプロイ先 Ryzen AI 5 340）。`start.ps1` の既定は **`-Backend auto`（NPU → Vulkan → CPU、health で受理）**。**NPU** は自作 Ryzen AI ONNX シム経由（`npu/npu_server.py`、ローカル `LFM2-1.2B-ONNX_rai_1.7.1`、`reason_decision_prompt`）。**Vulkan** は llama.cpp の Vulkan ビルドで内蔵Radeonにオフロード（`-ngl 99`）、**CPU** は `-ngl 0` フォールバック。`-Backend npu|vulkan|cpu` で単一強制（vulkan/cpuがNPU無効化）。**NVIDIA/CUDA は不要・非対象**。Windows の AMD iGPU は ROCm 非対応のため Vulkan を採用。Vulkan版 `llama-server` は `winget install ggml.llamacpp`、NPU は AMD NPU ドライバ + Ryzen AI 1.7.1（conda env `ryzen-ai-1.7.1`）+ ローカル LFM2 ONNX が前提（§8.5）。実測では開発機は GPU 最速だが、デプロイ先 340 は iGPU が約 1/10 で分類用途は NPU 優位の見込み（§8.5 / records）。 |
...
```text
Go Local DLP Proxy
  ↓ localhost HTTP (OpenAI互換, /v1/chat/completions, port 8792)
自作シム npu/npu_server.py
  ↓ onnxruntime.InferenceSession + RyzenAILightExecutionProvider
AMD LFM2 token-fusion ONNX → AMD Ryzen AI NPU (XDNA2)
```

理由:

* 推論ランタイムは依存関係が重く、Go本体に直接組み込むと保守性が下がる。
* CPU/iGPU(Vulkan)/NPU(OGA) バックエンドの差し替えを容易にする（同一 OpenAI 互換 HTTP 契約）。
...
Claude Code の Anthropic API 接続先をローカルプロキシへ向ける。
...
as-built（§8.7）では HTTP adapter（OpenAI互換）を採用する。Vulkan/CPU は llama.cpp `llama-server`、**NPU は自作 Ryzen AI ONNX シム**（`npu/npu_server.py`）を同じ HTTP 契約で叩く（§8.5）。バックエンド差はサイドカー起動オプションと、proxy への endpoint/モデル/プロファイルの上書きで吸収する。

### 8.5 AMD NPU対応（as-built）

llama.cpp / Ollama は NPU を駆動できない（GGUF=CPU/GPU のみ）。**LFM2 は AMD NPU 公式対応だが OGA 形式ではなく、AMD 独自の token-fusion ONNX グラフ**（`onnxruntime.InferenceSession` + `RyzenAILightExecutionProvider`、`Run-LFM2.py` 方式）で配布される。**Lemonade(OGA) は LFM2 を実行できない**（`genai_config.json` 必須＝OGA 専用、LFM2 は OGA フロー対象外）。`Run-LFM2.py` は CLI でサーバではないため、その prefill/decode ループを **自作の薄い OpenAI 互換シム**（`npu/npu_server.py`、Python 標準ライブラリ）で包み、llama.cpp と同じ HTTP 契約で提供する。

```text
Go Proxy
  -> localhost HTTP (OpenAI互換, /v1/chat/completions, port 8792)
  -> 自作シム npu/npu_server.py（Ryzen AI conda env で実行）
  -> onnxruntime.InferenceSession + RyzenAILightExecutionProvider
  -> AMD LFM2 token-fusion ONNX -> AMD Ryzen AI NPU (XDNA2)
```
...
```text
- 既定 start.ps1 -Backend auto: NPU -> Vulkan -> CPU を health で順に受理。
  NPU は /health に加え 1トークン補完まで確認（モデル未ロードを検出）。
  NPU利用不可/不調時は Vulkan -> CPU へ自動フォールバック。-Backend vulkan/cpu でNPU無効化。
- モデルは AMD プリビルド ONNX: amd/LFM2-1.2B-ONNX_rai_1.7.1（ローカル dir を -NpuModel で指定）。
- NPU は GBNF schema 非強制のため response_format を送らない profile=reason_decision_prompt を使用。
- 起動時に health check + warm-up（既存 LlamaClient.Health/Warmup を流用。シム側でも warmup）。
- 推論backend名(npu/vulkan/cpu)・モデル名を audit event に記録（backend は実ランタイムを記録）。
- 前提: AMD NPU ドライバ / Miniforge(conda) / Ryzen AI Software 1.7.1（env `ryzen-ai-1.7.1`）/ ローカル LFM2 ONNX。
- ベンチ: 開発機 Strix Halo では GPU 最速。デプロイ先 Ryzen AI 340 は iGPU 約 1/10 で分類用途は NPU 優位の見込み
  （docs/records/20260607/0125-npu-shim-and-benchmark.md）。
- 注意: 本番モデルは JP fine-tune（LFM2.5-1.2B-JP-...）を想定。NPU 版は AMD の量子化/コンパイル変換が必要で
  現状は stock LFM2-1.2B プリビルドのみ（変換は docs/todo.md に Deferred）。
```
...
> 改訂（§1.1）実装状況: **M1〜M5 は実装・テスト済み**（M2/M3 はマーカー/Registryではなくステートレス＋fingerprintキャッシュ＋構造認識ユニット除去として実現）。実LFM2.5での実機検証も完了。**M6（NPU）は自作 Ryzen AI ONNX シム（`npu/npu_server.py`）経由で実装済み**（実機で LFM2-on-NPU 動作・3経路ベンチ確認済み。Lemonade(OGA) は LFM2 を実行できないため不採用）。**M7（管理API本格版）は未着手**。
...
### Milestone 6: AMD NPU対応（as-built: 自作 Ryzen AI ONNX シム）

```text
- NPU は自作シム npu/npu_server.py（LFM2 token-fusion ONNX + RyzenAILightExecutionProvider）を
  OpenAI 互換 HTTP で利用。Lemonade(OGA) は LFM2 非対応のため不採用。
- start.ps1 -Backend auto: NPU -> Vulkan -> CPU の health gated フォールバック
- warm-up / health check は既存 LlamaClient を流用（NPU は 1トークン補完で readiness 確認）
- backend status: audit に実ランタイム(npu/vulkan/cpu)を記録
- 実機で LFM2-on-NPU 動作・3経路ベンチ確認済み（records 20260607/0125）
- 残: JP fine-tune の NPU 変換（Quark 量子化 + token-fusion ONNX/ctrlpkt 生成、docs/todo.md）と 340 実機での既定確認
```
